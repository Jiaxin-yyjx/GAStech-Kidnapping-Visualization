...
def create_pretraining_corpus(raw_text, nlp, file, file2, window_size=40):
    '''
    ...
    entities_of_interest = ["PERSON", "NORP", "FAC", "ORG", "GPE", "LOC", "PRODUCT", "EVENT", \
                            "WORK_OF_ART", "LAW", "LANGUAGE" ,"KIDNAP", "PROTEST", "FAMILY", "MEMBER", "CO-FOUNDER", \
                            "FOUNDER", "LEADER", "THREAT"]
    ...
    for idx in samples_D_idx:
        print(D[idx], '\n')
        file.writelines(str(D[idx]))
        file.writelines('\n')
        file2.writelines(str(D[idx][1]) + ',' + str(D[idx][2]) + '\n')
    ref_D = len(D)
...
def load_dataloaders(args, max_length=5000):
    file = open('out.txt', 'w')
    file2 = open('relation.txt', 'w')
    ...
    for text_chunk in tqdm(text_chunks, total=num_chunks):
        D.extend(create_pretraining_corpus(text_chunk, nlp, file, file2, window_size=40))
